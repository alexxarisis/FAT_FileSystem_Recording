 ...
 *  Fixes:
 *
 *	Max Cohan: Fixed invalid FSINFO offset when info_sector is 0F
 */
...
{
	// my changes
	printk(KERN_INFO "Inode: fat_writepage\n");
...
{
	// my changes
	printk(KERN_INFO "Inode: fat_writepages\n");
...
{
	// my changes
	printk(KERN_INFO "Inode: fat_readpage\n");
...
{
	// my changes
	printk(KERN_INFO "Inode: fat_readpages\n");
			...
			struct page **pagep, void **fsdata)
{
	...
	*pagep = NULL;
	...
	// my changes
	printk(KERN_INFO "Inode: fat_write_begin\n");
			...
			struct page *pagep, void *fsdata)
{
	struct inode *inode = mapping->host;
	int err;
	char buf[2048]; // just to be sure
	// my changes
	printk(KERN_INFO "Inode: fat_write_end\n");
...
static ssize_t fat_direct_IO(struct kiocb *iocb, struct iov_iter *iter)
{
	...
	// my changes
	printk(KERN_INFO "Inode: fat_direct_IO\n");
	...
	if (iov_iter_rw(iter) == WRITE) {
		...
		{
			printk(KERN_INFO "msdos_inode_info->mmu_private = %llu", size);
			sprintf(buf, "msdos_inode_info->mmu_private = %llu", size);
...
static sector_t _fat_bmap(struct address_space *mapping, sector_t block)
{
	...
	// my changes
	printk(KERN_INFO "Inode: _fat_bmap\n");
...
int fat_fill_inode(struct inode *inode, struct msdos_dir_entry *de)
{
	...
	printk(KERN_INFO "INODE initialization\n"
					 "\t\tnr_caches = %d\n"
					 "\t\ti_start = %d\n"
					 "\t\ti_logstart = %d\n"
					 "\t\ti_attrs = %d\n"
	...
	sprintf(buf, "INODE initialization\n"
					 "\t\tnr_caches = %d\n"
					 "\t\ti_start = %d\n"
					 "\t\ti_logstart = %d\n"
					 "\t\ti_attrs = %d\n"
...
{
	// my changes
	printk(KERN_INFO "Superblock: fat_evict_inode\n");
...
{
	struct msdos_sb_info *sbi = MSDOS_SB(sb);
	char buf[4096]; // just to be sure
	// my changes
	printk(KERN_INFO "Superblock: fat_put_super\n");
	...
	printk(KERN_INFO "SUPERBLOCK : fat_put_super\n"
					 ...
					 "\t\tsbi->fats = %u\n\t\tsbi->fat_bits = %u\n\t\tsbi->fat_start = %u\n\t\tsbi->fat_length = %lu\n"
					 "\t\tsbi->dir_start = %lu\n\t\tsbi->dir_entries = %u\n\t\tsbi->data_start = %lu\n"
					 "\t\tsbi->max_cluster = %lu\n\t\tsbi->root_cluster = %lu\n\t\tsbi->fsinfo_sector = %lu\n"
					 "\t\tsbi->prev_free = %u\n\t\tsbi->free_clusters = %u\n\t\tsbi->free_clus_valid = %u\n"
					 "\t\tsbi->dir_ops = %p\n\t\tsbi->dir_per_block = %u\n\t\tsbi->dir_per_block_bits = %u\n"
					 "\t\tsbi->vol_id = %u\n\t\tsbi->dirty = %u\n\t\tsbi->fatent_shift = %u\n--------------------------\n",
	...
	sprintf(buf, "SUPERBLOCK : fat_put_super\n"
					 "\t\tsbi->sec_per_clus = %u\n\t\tsbi->cluster_bits = %u\n\t\tsbi->cluster_size = %u\n"
					 "\t\tsbi->fats = %u\n\t\tsbi->fat_bits = %u\n\t\tsbi->fat_start = %u\n\t\tsbi->fat_length = %lu\n"
					 "\t\tsbi->dir_start = %lu\n\t\tsbi->dir_entries = %u\n\t\tsbi->data_start = %lu\n"
					 "\t\tsbi->max_cluster = %lu\n\t\tsbi->root_cluster = %lu\n\t\tsbi->fsinfo_sector = %lu\n"
...
{	
	...
	// my changes
	printk(KERN_INFO "Superblock: fat_alloc_inode\n");
...
{
	// my changes
	printk(KERN_INFO "Superblock: fat_destroy_inode\n");
	sys_close(MSDOS_SB(inode->i_sb)->recordFile);
...
{	
	...
	*flags |= MS_NODIRATIME | (sbi->options.isvfat ? 0 : MS_NOATIME);
	...
	// my changes
	printk(KERN_INFO "Superblock: fat_remount\n");	
	return 0;
}
...
static int fat_statfs(struct dentry *dentry, struct kstatfs *buf)
{
	...
	// my changes
	printk(KERN_INFO "Superblock: fat_statfs\n"); // changes
...
static int fat_write_inode(struct inode *inode, struct writeback_control *wbc)
{
	...
	// my changes
	printk(KERN_INFO "Superblock: fat_write_inode\n");
...
static int fat_show_options(struct seq_file *m, struct dentry *root)
{
	...
	// my changes
	printk(KERN_INFO "Superblock: fat_show_options\n");
		   ...
		   void (*setup)(struct super_block *))
{
	...
	// my changes
	printk(KERN_INFO "Superblock Initialization.\n");
	sbi->recordFile = sys_open("recording file", O_RDWR | O_CREAT | O_APPEND, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH);
	...
	// buf variable already exists
	sprintf(buffer, "SUPERBLOCK initialization\n"
					 "\t\tsbi->sec_per_clus = %u\n\t\tsbi->cluster_bits = %u\n\t\tsbi->cluster_size = %u\n"
					 "\t\tsbi->fats = %u\n\t\tsbi->fat_bits = %u\n\t\tsbi->fat_start = %u\n\t\tsbi->fat_length = %lu\n"
					 "\t\tsbi->dir_start = %lu\n\t\tsbi->dir_entries = %u\n\t\tsbi->data_start = %lu\n"
					 "\t\tsbi->max_cluster = %lu\n\t\tsbi->root_cluster = %lu\n\t\tsbi->fsinfo_sector = %lu\n"
	...
	sys_write(sbi->recordFile, buffer, sizeof(buffer));
	sys_fsync(sbi->recordFile);
	sys_fdatasync(sbi->recordFile);		  
	return 0;
